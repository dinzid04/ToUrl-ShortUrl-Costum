...
const StatsBar = ({ history, shortHistory }: { history: HistoryItem[], shortHistory: any[] }) => {
  const totalUploads = history.reduce((acc, item) => acc + (item.files?.length || 1), 0);
  const totalShorts = shortHistory.length;
  const totalSize = history.reduce((acc, item) => {
...
export default function App() {
  ...
  const refreshStats = () => {
    ...
    if (savedShorts) {
      try { setShortHistory(JSON.parse(savedShorts)); } catch (e) {}
    }
  };
  ...
  useEffect(() => {
    ...
    if (savedUploads) {
      try { setHistory(JSON.parse(savedUploads)); } catch (e) {}
    }
    refreshStats();
