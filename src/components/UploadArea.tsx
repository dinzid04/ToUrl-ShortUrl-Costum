...
export default function UploadArea({ onUploadSuccess }: UploadAreaProps) {
    ...
    try {
      ...
      if (!contentType || !contentType.includes("application/json")) {
        ...
        if (response.status === 413) {
          ...
          throw new Error('Waktu koneksi habis (Timeout). File mungkin terlalu besar.');
        }
  ...
  return (
    <div className="w-full relative z-10">
      <div className="flex bg-zinc-950/60 border border-white/10 p-1.5 rounded-2xl w-full mb-8 shadow-[0_0_15px_rgba(0,0,0,0.1)] relative">
        <button
        ...
        >
          ...
          <span className="text-[15px] z-10 tracking-wide font-semibold">Private Server</span>
          ...
          <div className="z-10 mt-1 h-5 flex items-center justify-center">
            {storageType === 'private' ? (
              <select
                 ...
                 className="text-[11px] w-auto bg-black/30 border border-white/20 rounded-md outline-none text-blue-300 px-2 py-0.5 cursor-pointer hover:bg-black/50 transition-colors"
              >
                 <option value="0" className="bg-zinc-900">Perpetual (Tanpa Expire)</option>
                 <option value="1" className="bg-zinc-900">Hapus Otomatis di 1 Jam</option>
                 <option value="12" className="bg-zinc-900">Hapus Otomatis di 12 Jam</option>
                 <option value="24" className="bg-zinc-900">Hapus Otomatis di 24 Jam</option>
                 <option value="168" className="bg-zinc-900">Hapus Otomatis di 7 Hari</option>
              </select>
            ) : (
              <span className="text-[10px] text-zinc-500">Google Drive / Catbox</span>
            )}
          </div>
        </button>
        <button
        ...
        >
          ...
          <span className="text-[15px] z-10 tracking-wide font-semibold">Cloud Sementara</span>
          <div className="z-10 mt-1 h-5 flex items-center justify-center">
            <span className="text-[10px] text-zinc-500">Hapus Sendiri di 24 Jam</span>
          </div>
        </button>
      </div>
